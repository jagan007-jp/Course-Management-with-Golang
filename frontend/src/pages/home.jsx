...
function CourseGrid() {
  ...
  const searchText = useSelector((state)=>state.user.userSearch)
  useEffect(() => {
  ...
  const fetchFavourites = async () => {
    try {
      const res = await fetch(`http://localhost:8080/fav/get/${username}`,{
        headers:{
          Authorization : `${token}`
        }
      });
  ...
  const fetchCourses = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        `http://localhost:8080/courses?limit=${limit}&offset=${offset}`
      ,{
        headers:{
          Authorization : `${token}`
        }
  ...
  const handleDelete = async (course) =>{
    try{
    const res = await fetch(`http://localhost:8080/courses/delete/${course.id}`,{
      headers:{
          Authorization : `${token}`
      },
      method: "DELETE"
  ...
  const handleFav = async (course) => {
    ...
    try {
      const url = isAdding
        ? "http://localhost:8080/fav/add"
        : `http://localhost:8080/fav/${course.id}`;
  ...
  return (
    <>
    {isAdmin && (<Button colorScheme={"green"} float={"right"} margin={"10px"} onClick={()=> navigate("/addCourse")}>Add Courses</Button>)}
    <Container maxW="container.lg" mt={10}>
            ...
            >
              ...
              <Stack p={5} spacing={4} flex="1">
                ...
                <Flex gap={3} mt="auto">
                  ...
                  {isAdmin && (
                    <Button onClick={()=> handleDelete(course)} colorScheme="red"><MdDelete /></Button>
                  )}
                </Flex>
              </Stack>
            </Box>
